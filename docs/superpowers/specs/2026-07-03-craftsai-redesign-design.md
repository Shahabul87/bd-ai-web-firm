...
1. **Hero — "The Drafting Room"** *(supersedes the earlier "Foundry Floor" concept; founder picked this from four browser-previewed directions on 2026-07-03; implemented in `components/home/DraftingRoomHero.tsx`)*: the hero sits on its own surface — deep drafting-blue `#0D2038` with a chalk `#EDEDE3` drafting-paper grid and vignette. Headline in **Archivo Expanded** (self-hosted, hero-only face): `We draft agents.` / `The agents build the rest.` with amber `#FFB347` highlight. A chalk SVG agent-schematic (BRIEF → AGENT → WEBSITE/APP/INTEGRATION) draws itself on load via framer-motion pathLength, then amber dimension marks annotate it; reduced motion renders the finished drawing instantly (hydration-safe: effect-started animation). Amber CTA `Start a project` + chalk ghost `See the process`. Mono fig-caption: `fig. 01 — agent schematic · dhaka / worldwide`. Replaces all three legacy heroes and the Three.js scene.
